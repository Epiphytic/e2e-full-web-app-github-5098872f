...
The `x509-cert` and `pem` crates are required to extract the RSA public key from the self-signed X.509 certificate (`cert.pem`) generated by `generate-keys.sh`. The `rcgen` dev-dependency generates ephemeral certificates for unit tests.
...
Note: The `generate-keys.sh` script (Step 1) produces three artifacts: `private.pem` (RSA private key), `public.pem` (RSA public key), and `cert.pem` (self-signed X.509 certificate). The certificate satisfies the local JWT CA requirement — the server validates tokens by extracting the public key from `cert.pem` rather than using `public.pem` directly.
...
> **Split rationale:** Originally a single CRUISE-004 task covering connection pooling, DDL operations, and query helpers. Split into 004a (connection pool + read-only queries) and 004b (DDL operations + identifier validation) because the DDL operations are security-sensitive (SQL injection risks from dynamic DDL construction) and significantly more complex than the basic connection/query logic. This separation enables: (1) parallel execution and independent review of the security-critical DDL code, (2) a clearer security boundary — 004a has no SQL injection surface since it only reads schema metadata via SQLite PRAGMAs, while 004b requires strict identifier validation for all user-provided names, and (3) faster unblocking of 004b's focused security review without waiting for connection pool work.
...
> **Split rationale:** Separated from CRUISE-004a because DDL operations (CREATE TABLE, DROP TABLE, ALTER TABLE ADD/DROP COLUMN) dynamically construct SQL from user-provided identifiers, creating a direct SQL injection attack surface. This task requires strict identifier validation (alphanumeric + underscore only, reserved word rejection) and column type whitelisting — security concerns that are absent from 004a's read-only PRAGMA-based queries. Isolating this code in its own task and spawn instance (SPAWN-003b) allows focused security review of the injection prevention logic independently from the basic connection pool and query infrastructure.
