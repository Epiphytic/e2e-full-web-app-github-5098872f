...
- **E2E tests parallelized by feature area (CRUISE-008a/008b/008c split):** The original monolithic CRUISE-008 (high complexity) was split into three independent tasks — Auth (008a), Table (008b), and Column (008c) E2E tests — each at medium complexity. This enables parallel implementation and verification: Auth E2E tests (008a) depend only on CRUISE-006a and can begin as soon as auth handlers are wired, without waiting for table/column handlers or the full DB layer. Table (008b) and Column (008c) E2E tests depend on CRUISE-006b and can be implemented in parallel with each other once handlers are ready. Each split task runs in its own spawn instance (SPAWN-006b, 006c, 006d) for concurrent execution.
